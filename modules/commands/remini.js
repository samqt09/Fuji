...
exports.run = async (o) => {
  ...
  send(`Increasing the resolution for ${o.event.messageReply.attachments.length} image (${o.event.messageReply.attachments.length * eta}s)`);

  let stream = [];
  let exec_time = 0;
  for (let i of o.event.messageReply.attachments) {
    try {
      let res = await require('axios').get(encodeURI(`https://api.remini.ai/v1/upscale.png?{"image":"${i.url}","model":"4x-UltraSharp"}`), {
        responseType: 'stream',
      });
