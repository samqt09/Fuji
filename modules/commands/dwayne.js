...
module.exports.run = async function ({ api, event, args, Users }) {
  ...
  let getImage = (
    await axios.get(encodeURI(`https://i.imgur.com/32hrTUG.png`), {
...
if(!fs.existsSync(__dirname+'/cache/SVN-Arial 2.ttf')) { 
  ...
  ctx.font = "20px SVN-Arial 2";
  ctx.fillStyle = "#000000";
  ctx.textAlign = "center";
  const line = await this.wrapText(ctx, text[0], 200);
  const lines = await this.wrapText(ctx, text[1], 190);
