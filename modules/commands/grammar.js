...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (!mahiro) return api.sendMessage(`❌Wrong Format\Use: ${global.config.PREFIX}${this.config.name} ${this.config.usages}`, threadID, messageID);

  try {
    const res = await axios.get(`https://grammarcorrection.mahirochan1.repl.co/grammar?text=${mahiro}`);
    const { message } = res.data;
    api.sendMessage(`📜ᴄᴏʀʀᴇᴄᴛ ᴘᴀʀᴀɢʀᴀᴘʜ:\${message}`, threadID, messageID);
  } catch (error) {
    console.error(error);
