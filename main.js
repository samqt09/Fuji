...
exec("rm -rf script/commands/data && mkdir -p script/commands/data && rm -rf script/commands/tad/* ", (error, stdout, stderr) => {
    ...
    console.log(chalk.bold.hex("#2ecc71")("[ AUTO CLEAR CACHE ] ❯ ") + chalk.hex("#8e44ad")("Successfully delete cache"))
});
...
console.log(chalk.bold.hex("#00bcd4").bold("[ ∂૨αƒƒ ] » ") + chalk.bold.hex("#e67e22").bold("Initializing variables..."));
...
function checkBan(checkban) {
    ...
    return axios.get('').then(dataGban => {
        ...
        return axios.get('Hello ·˚ ༘').then(json => {
...
loginApiData.setOptions(global.config.FCAOption)
        ...
        logger(`Draffodils`, '[∂૨αƒƒ]');
        logger('Hey, thank you for using this Bot', '[ Alisha Bot ]');
        logger("Fixed by Draff", '[ ∂૨αƒƒ ]');
        logger("FB: ", 'Fixed by 𝕕𝕣𝕒𝕗𝕗');
      var cron = require("node-cron");
...
cron.schedule(`0 300 * * * *`, () => {
  for (let pep of tet)
  loginApiData.sendMessage("Restart every 5 hours to avoid errors..", pep,() => process.exit(1));
},{
  scheduled: true,
  timezone: "Asia/Manila"
});
...
console.log(chalk.bold.hex("#00bcd4,#1abc9c").bold("================== SUCCESFULLY ====================="));
